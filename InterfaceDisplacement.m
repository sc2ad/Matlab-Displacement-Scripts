function varargout = InterfaceDisplacement(varargin)
%INTERFACEDISPLACEMENT M-file for InterfaceDisplacement.fig
%      INTERFACEDISPLACEMENT, by itself, creates a new INTERFACEDISPLACEMENT or raises the existing
%      singleton*.
%
%      H = INTERFACEDISPLACEMENT returns the handle to a new INTERFACEDISPLACEMENT or the handle to
%      the existing singleton*.
%
%      INTERFACEDISPLACEMENT('Property','Value',...) creates a new INTERFACEDISPLACEMENT using the
%      given property value pairs. Unrecognized properties are passed via
%      varargin to InterfaceDisplacement_OpeningFcn.  This calling syntax produces a
%      warning when there is an existing singleton*.
%
%      INTERFACEDISPLACEMENT('CALLBACK') and INTERFACEDISPLACEMENT('CALLBACK',hObject,...) call the
%      local function named CALLBACK in INTERFACEDISPLACEMENT.M with the given input
%      arguments.
%
...
% Edit the above text to modify the response to help InterfaceDisplacement

% Last Modified by GUIDE v2.5 24-Jan-2016 20:15:59

% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
                   'gui_Singleton',  gui_Singleton, ...
                   'gui_OpeningFcn', @InterfaceDisplacement_OpeningFcn, ...
                   'gui_OutputFcn',  @InterfaceDisplacement_OutputFcn, ...
                   'gui_LayoutFcn',  [], ...
                   'gui_Callback',   []);
...
% global exit;
% exit=2;
% --- Executes just before InterfaceDisplacement is made visible.
function InterfaceDisplacement_OpeningFcn(hObject, eventdata, handles, varargin)
...
% Choose default command line output for InterfaceDisplacement
handles.output = hObject;

% Update handles structure
guidata(hObject, handles);

% UIWAIT makes InterfaceDisplacement wait for user response (see UIRESUME)
% uiwait(handles.figureInterfaceDisplacement);


% --- Outputs from this function are returned to the command line.
function varargout = InterfaceDisplacement_OutputFcn(hObject, eventdata, handles)
...
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
run('InterfaceDataAnalysis.m');
delete(handles.figureInterfaceDisplacement);
...
uiwait(msgbox('Draw a rectangle over the desired area. Double click the rectangle to run the measurements.', 'Instructions'));
